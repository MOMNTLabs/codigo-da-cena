...
const programDetails = [
  ["ENCONTROS", "2 DOMINGOS", "calendar"],
  ["DURAÇÃO", "8 HORAS POR ENCONTRO", "clock"],
  ["EXPERIÊNCIA", "CAFÉ DA MANHÃ + COFFEE BREAK", "coffee"],
  ["PARTICIPAÇÕES", "ARTISTAS NACIONAIS", "microphone"],
  ["TURMA", "ATÉ 50 DJs", "group"],
  ["GRAVAÇÕES", "PRODUÇÃO PROFISSIONAL · ACESSO EXCLUSIVO AOS ALUNOS", "recording"],
] as const;

function ProgramDetailIcon({ kind }: { kind: string }) {
  const common = { className: "program-detail-icon", viewBox: "0 0 64 64", fill: "none", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round" as const, strokeLinejoin: "round" as const, "aria-hidden": true };
  if (kind === "calendar") return <svg {...common}><rect x="10" y="14" width="44" height="40" rx="5" /><path d="M10 25h44M21 9v10M43 9v10" /><circle cx="25" cy="38" r="3" /><circle cx="39" cy="38" r="3" /></svg>;
  if (kind === "clock") return <svg {...common}><circle cx="32" cy="32" r="23" /><path d="M32 18v15l10 6M32 9V5M55 32h4M32 55v4M9 32H5" /></svg>;
  if (kind === "coffee") return <svg {...common}><path d="M14 24h32v12a15 15 0 0 1-15 15h-2a15 15 0 0 1-15-15V24Z" /><path d="M46 29h4a7 7 0 0 1 0 14h-7M22 18c-3-4 3-6 0-10M32 18c-3-4 3-6 0-10M42 18c-3-4 3-6 0-10" /></svg>;
  if (kind === "microphone") return <svg {...common}><rect x="22" y="7" width="20" height="34" rx="10" /><path d="M15 31a17 17 0 0 0 34 0M32 48v10M23 58h18M22 22h20" /></svg>;
  if (kind === "group") return <svg {...common}><circle cx="32" cy="20" r="8" /><circle cx="14" cy="29" r="6" /><circle cx="50" cy="29" r="6" /><path d="M18 53c1-11 6-17 14-17s13 6 14 17M5 53c1-9 4-14 10-14 3 0 5 1 7 3M59 53c-1-9-4-14-10-14-3 0-5 1-7 3" /></svg>;
  return <svg {...common}><rect x="8" y="14" width="48" height="36" rx="5" /><path d="m27 24 14 8-14 8V24ZM19 8h26" /><circle cx="15" cy="8" r="2" /><circle cx="49" cy="8" r="2" /></svg>;
}
